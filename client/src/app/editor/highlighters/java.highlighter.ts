...
export class JavaHighlighter implements Highlighter {
    ...
    private handleMultiLineComment(currentIndex: number, builder: any[]): void {
        let endOfMultiLineComment = this.getEndOfMultiLineComment(this.isMultiCommentOpened ? currentIndex : currentIndex + 2);
    ...
    private appendSubstring(start: number, end: number, styleClass?: string): void {
        const text = this.createTextElement(this.code.substring(start, end), styleClass !== null || styleClass !== 'undefined');
